...
export default function ModalFinishedGame({ player, gameState }: Props) {
  return (
    <ModalTemplate>
        ...
        <div className="text-xs sm:text-sm md:text-base">
          {gameState?.roundRecap?.playersWhoWinMatch.length == 1 && (
            <div>
              ...
              <span
                className={`text-${gameState.roundRecap.playersWhoWinMatch[0].color}`}
              >
                {gameState.roundRecap.playersWhoWinMatch[0].userName}
              </span>
            </div>
          )}
          {gameState?.roundRecap?.playersWhoWinMatch &&
            gameState?.roundRecap?.playersWhoWinMatch.length > 1 && (
              <div>
